...
★ 5【再分配会把"减仓"放大成"加仓"】（§5.3 没写、但一定会发生）
  可交易部分按 `(π − L)/Σ_{j∉F} w^tgt_j` 统一放缩。被拦住的买入越多，这个系数越大于 1，
  于是某只本来只想小幅减仓的票会被推到 `w_prev` 之上 —— 变成买入。若它正一字涨停
  （初判"想卖"故未入 F），天真的实现就在涨停板上买进了。所以 F 必须迭代到不动点：
  F 单调增长，最坏情形全员入 F（此时无任何成交、无从违规），故必然收敛。
  与 `portfolio._violators` 的循环是同一个套路。
...
# 退市清仓折价（架构 B8）。退市整理期连续跌停、几乎无流动性，按收盘价成交是系统性乐观偏差。
# B8 的字面口径是"进入退市整理期【首日】收盘 × 0.5"，而掩码只给得出执行日（或最后一根
# 有效 K 线）的收盘价 —— 后者已在整理期的跌停序列末端，比 B8 更保守，符合"宁可低估"。
_DELIST_HAIRCUT = 0.5
...
def _allocate(tgt: pd.Series, prev_w: pd.Series, locked: pd.Series, pi: float) -> pd.Series:
    """§5.3：不可交易的锁在 w_prev，可交易的按目标【相对比例】分配剩余额度 (π − L)。
    ...
    return w if denom <= 0 else w + free * (budget / denom)
