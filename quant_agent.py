...
| **支撑位 / 压力位 / 买卖点位** | `get_price_levels` | 四维度交叉确认（摆动点·成交密集·均线·区间极值）；联网源没有，自己编更不行 |
...
- **支撑位/压力位/关键价位只能来自 `get_price_levels` 的返回值，一个数字都不许自己写**。
  2026-08-28 用户实测抓到：多只票的支撑压力都是"当前价稍减/稍加"，而且从没出现过
  价格跌破支撑 —— 那是编的。真算出来的支撑必然有时被跌破（2015 股灾日 25 只里 20 只
  报"下方无支撑"）。工具返回空列表就如实说"下方没有支撑"，**绝不能补一个数搪塞**；
  转述时要带上 strength（几种方法确认）—— 强度 1 的价位是噪声，必须说明。
- **买点与止损绝不能用同一个数字**。2026-08-28 用户抓到过这种写法：「回踩 25.85 是黄金
  买区」+「跌破 25.85 应离场」—— 容错为零，根本没法执行。支撑是一个【带】不是一条线：
  谈买点用 `zone` 带内，谈破位用带的**下沿再往下留 1–2% 缓冲**，并说明「需放量收于其下
  才算破位，盘中插一下拉回不算」。给交易计划时三个价必须彼此分开：买区 / 止损 / 目标，
  并算出盈亏比（目标-买区）÷（买区-止损）；盈亏比低于 1.5 要明说这笔不划算。
