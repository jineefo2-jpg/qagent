...
### 烧前预注册清单（2026-08-27 15:0x 冻结，用户已批准；执行脚本 `scripts/oos_ceremony_2026-08-27.py`）

一切口径在看到任何样本外数字**之前**写死于此；烧后只许在下方补「结果」，不许改判据。

- **数据**：快照 `c2cd172976e1f3fe`，行情迄 2026-08-21（定时器已摘除护场，仪式期间零 promote）。
- **样本外区间**：2020-01-01 → 2026-08-21（闸 1 内部按 `OOS_CUTOFF=2019-12-31` 切分）。
- **两臂**（选股层完全相同：15 个 alpha 等权、约束默认 top_n=50、成本默认）：
  A = 恒定仓位 `macro_timing=False, position_cap=0.8`；
  B = 宏观择时 `macro_timing=True, floor=0.2, cap=1.0`。
- **判据（冻结）**：
  ① 关停（规格 §6.1 原文）：B 存活当且仅当 **Sharpe_oos(B) > Sharpe_oos(A) 且
     Calmar_oos(B) > Calmar_oos(A)**；任一不超 → π ≡ 0.8 常数化，宏观层降级纯展示。
  ② 闸 1 施加于**胜出臂**：`SR_oos ≥ 0.6 × SR_is`。不过 → 策略不投产，回研究阶段。
  ③ 生产配置 = 胜出臂的配置；每晚 CLI 参数随之定死。
- **烧前前置（过夜，全绿才烧）**：闸 2/4/5（网格 `{"top_n": [30, 50, 80]}` ——
  §6.2「N 默认 50 需通过参数高原检验」的字面兑现）→ 闸 3（n=200, seed=0）→
  样本外区间因子落库。闸 2–5 跑在 A 臂 config 上，合法性：两臂选股层逐位相同，
  π 是常量缩放，不影响选参与同日横截面置换的语义。
- **烧的动作**：脚本无条件连跑 `gate1(A)`、`gate1(B)` 两次（不许看完一臂再决定跑不跑另一臂），
  两行样本外台账自动落 `docs/oos-runs.md`，判定按 ①② 机械得出。

### 结果（2026-08-28 04:05 执行，机会已用尽）

| | 样本内 Sharpe | 样本外 Sharpe | 门槛 | 闸 1 |
|---|---|---|---|---|
| A 恒定 0.8 | 0.4891 | **0.4156** | 0.2934 | 过（保住 85%） |
| B 宏观择时 | 0.4835 | 0.3929 | 0.2901 | 过（保住 81%） |

**判定**：① 宏观择时层**关停**（Sharpe 0.3929 < 0.4156，「且 Calmar」当场不成立，
Calmar 未算 —— 事后补算不影响结论的数字是给自己留腾挪空间）→ π ≡ 0.8；
② 闸 1 施加于胜出臂 A **过** → 策略可投产；③ 生产配置 = A 臂
（`macro_timing=False, position_cap=0.8`，param_hash `ab102248f1b54f7f` /
样本外段 `601ade2d15ac96f9`）。全文见 `docs/oos-runs.md`。

**烧之前修掉的两个引擎缺陷**（首次 burn 在 OOS 段抛异常中止，**未产出任何样本外数字**，
机会未消耗；修复后重跑才是上表）：`preload` 换窗不清 `_PRELOAD_WIDE`（同进程连跑两次
回测时第二次读到第一次那个窗的价格面板 —— 我 08-27 性能专项引入的）；引擎在「日历有
T+1、行情没有」时照样成交（裁决 ⑦ 只写进文档从未实现）。两条均配先失败的回归测试；
闸 2/4/5 修复后复跑与修复前**逐字一致**，证明四闸结论未被污染。
