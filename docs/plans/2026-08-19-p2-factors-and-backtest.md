...
>
> **同一处 surgery 还压着第二笔账**：开了诊断后每个调仓日**每个因子要算两遍**
> （`combine` 里一遍出合成分数，`compute_panel` 里一遍出 IC 面板），外加 `log_mv`。
> 一次过的修法是让 `combine` 顺带交出逐因子的 processed 面板 ——
> 和 `use_store` 是同一层、同一次改动，**两件事一起裁**。
>
...
> **★ 2026-08-24 裁决（Task 14 转来）：`append_oos_run(result, *, sharpe_is=None)`。**
> `store.append_oos_run` 的文档承诺「样本内 Sharpe 由闸 1 成对写入」，
